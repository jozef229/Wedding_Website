<template>
    ...
    <div v-if="isOpen" class="align-middle w-full block flex-grow lg:flex lg:items-center lg:w-auto">
        ...
        <a class="px-4 border-gray-600 lg:border-r-2 text-gray-300 block mt-4 lg:inline-block lg:mt-0 text-teal-200 hover:text-white hover:underline" @click="backHost()">{{ $t('navigation_host') }}</a>
...
export default {
    ...
    methods: {
        backHost(){
            window.location.href = "/host/" + this.$route.params.pass
        },
        goto(el) {
